...
namespace KerbalConstructionTime
{
    ...
    public class KCT_Preset
    {
        ...
        private int[] upgradesInternal;
        public int[] StartUpgrades
        {
            get
            {
                if (upgradesInternal == null)
                {
                    upgradesInternal = new int[3] {0, 0, 0}; //career, science, sandbox
                    string[] upgrades = generalSettings.StartingPoints.Split(',');
                    for (int i = 0; i < 3; i++)
                        if (!int.TryParse(upgrades[i], out upgradesInternal[i]))
                            upgradesInternal[i] = 0;
                }
                return upgradesInternal;
            }
        }
