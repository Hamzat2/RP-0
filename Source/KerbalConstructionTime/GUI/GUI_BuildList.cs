...
namespace KerbalConstructionTime
{
    public static partial class KCT_GUI
    {
        ...
        private static int _listWindow = -1;
        private static bool _isVABSelected, _isSPHSelected, _isTechSelected;
        ...
        public static void DrawBuildListWindow(int windowID)
        {
            ...
            IKCTBuildItem buildItem = Utilities.NextThingToFinish();
            if (buildItem != null)
            {
                string txt = buildItem.GetItemName(), locTxt = "VAB";
                if (buildItem.GetListType() == BuildListVessel.ListType.Reconditioning)
                {
                    Recon_Rollout reconRoll = buildItem as Recon_Rollout;
                    if (reconRoll.RRType == Recon_Rollout.RolloutReconType.Reconditioning)
                    {
                        txt = "Reconditioning";
                        locTxt = reconRoll.launchPadID;
                    }
                    else if (reconRoll.RRType == Recon_Rollout.RolloutReconType.Rollout)
                    {
                    ...
                    else if (reconRoll.RRType == Recon_Rollout.RolloutReconType.Rollback)
                    {
                        BuildListVessel associated = reconRoll.KSC.VABWarehouse.FirstOrDefault(blv => blv.Id.ToString() == reconRoll.associatedID);
            ...
            if (VABSelectedNew != _isVABSelected)
                ...
                SelectList("SPH");
            else if (TechSelectedNew != _isTechSelected)
                SelectList("Tech");
            ...
            if (_listWindow == 0) //VAB Build List
            {
                RenderVABBuildList();
            }
            else if (_listWindow == 1) //SPH Build List
            {
                RenderSPHBuildList();
            }
            else if (_listWindow == 2) //Tech nodes
            {
                RenderTechList();
            }
        ...
        private static void RenderVABBuildList()
        {
            ...
            foreach (Recon_Rollout reconditioning in KCTGameStates.ActiveKSC.Recon_Rollout.FindAll(r => r.RRType == Recon_Rollout.RolloutReconType.Reconditioning))
            {
                GUILayout.BeginHorizontal();
                ...
                GUILayout.Label(reconditioning.ProgressPercent().ToString() + "%", GUILayout.Width(_width1 / 2));
            ...
            _scrollPos = GUILayout.BeginScrollView(_scrollPos, GUILayout.Height(250));
            {
                ...
                for (int i = 0; i < buildList.Count; i++)
                {
                    ...
                    GUILayout.Label(Math.Round(b.ProgressPercent(), 2).ToString() + "%", GUILayout.Width(_width1 / 2));
                    if (b.BuildRate > 0)
                        GUILayout.Label(MagiCore.Utilities.GetColonFormattedTime(b.TimeLeft), GUILayout.Width(_width2));
                    else
                        GUILayout.Label("Est: " + MagiCore.Utilities.GetColonFormattedTime((b.BuildPoints + b.IntegrationPoints - b.Progress) / Utilities.GetBuildRate(0, BuildListVessel.ListType.VAB, null)), GUILayout.Width(_width2));
                    GUILayout.EndHorizontal();
                }

                //ADD Storage here!
                buildList = KCTGameStates.ActiveKSC.VABWarehouse;
                GUILayout.Label("__________________________________________________");
                GUILayout.Label("VAB Storage");
                if (Utilities.IsVabRecoveryAvailable() && GUILayout.Button("Recover Active Vessel"))
                {
                ...
                bool rolloutEnabled = PresetManager.Instance.ActivePreset.generalSettings.ReconditioningTimes &&
                ...
                for (int i = 0; i < buildList.Count; i++)
                {
                    ...
                    Recon_Rollout rollout = KCTGameStates.ActiveKSC.GetReconRollout(Recon_Rollout.RolloutReconType.Rollout, launchSite);
                    Recon_Rollout rollback = KCTGameStates.ActiveKSC.Recon_Rollout.FirstOrDefault(r => r.associatedID == b.Id.ToString() && r.RRType == Recon_Rollout.RolloutReconType.Rollback);
                    Recon_Rollout recovery = KCTGameStates.ActiveKSC.Recon_Rollout.FirstOrDefault(r => r.associatedID == b.Id.ToString() && r.RRType == Recon_Rollout.RolloutReconType.Recovery);
                    ...
                    bool siteHasActiveRolloutOrRollback = rollout != null || KCTGameStates.ActiveKSC.GetReconRollout(Recon_Rollout.RolloutReconType.Rollback, launchSite) != null;
                    if (rolloutEnabled && !HighLogic.LoadedSceneIsEditor && recovery == null && !siteHasActiveRolloutOrRollback) //rollout if the pad isn't busy
                    {
                        ...
                        Recon_Rollout tmpRollout = new Recon_Rollout(b, Recon_Rollout.RolloutReconType.Rollout, b.Id.ToString(), launchSite);
                        if (tmpRollout.cost > 0d)
                            GUILayout.Label("√" + tmpRollout.cost.ToString("N0"));
                        string rolloutText = (i == _mouseOnRolloutButton ? MagiCore.Utilities.GetColonFormattedTime(tmpRollout.GetTimeLeft()) : "Rollout");
                        ...
                        if (Event.current.type == EventType.Repaint)
                            if (GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition))
                                _mouseOnRolloutButton = i;
                            else if (i == _mouseOnRolloutButton)
                            ...
                            if (facilityChecks.Count == 0)
                            {
                                ...
                                else if (Utilities.ReconditioningActive(null, launchSite))
                                {
                                    ...
                                    ScreenMessage message = new ScreenMessage("[KCT] Cannot launch while LaunchPad is being reconditioned. It will be finished in "
                                        + MagiCore.Utilities.GetFormattedTime(((IKCTBuildItem)KCTGameStates.ActiveKSC.GetReconditioning(launchSite)).GetTimeLeft()), 4.0f, ScreenMessageStyle.UPPER_CENTER);
            ...
            _scrollPos = GUILayout.BeginScrollView(_scrollPos, GUILayout.Height(250));
            {
                ...
                for (int i = 0; i < buildList.Count; i++)
                {
                    ...
                    if (b.BuildRate > 0)
                        ...
                        GUILayout.Label("Est: " + MagiCore.Utilities.GetColonFormattedTime((b.BuildPoints + b.IntegrationPoints - b.Progress) / Utilities.GetBuildRate(0, BuildListVessel.ListType.SPH, null)), GUILayout.Width(_width2));
                    //GUILayout.Label(Math.Round(b.buildPoints, 2).ToString(), GUILayout.Width(width1 / 2 + 10));
                    GUILayout.EndHorizontal();
                }

                buildList = KCTGameStates.ActiveKSC.SPHWarehouse;
                GUILayout.Label("__________________________________________________");
                ...
                for (int i = 0; i < buildList.Count; i++)
                {
                    ...
                    if (HighLogic.LoadedScene != GameScenes.EDITOR && recovery == null && airlaunchPrep == null && AirlaunchTechLevel.AnyUnlocked())
                    {
                        ...
                        string airlaunchText = i == _mouseOnAirlaunchButton ? MagiCore.Utilities.GetColonFormattedTime(tmpPrep.GetTimeLeft()) : "Prep for airlaunch";
                        ...
                        if (Event.current.type == EventType.Repaint)
                            if (GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition))
                                _mouseOnAirlaunchButton = i;
                            else if (i == _mouseOnAirlaunchButton)
        ...
        private static void RenderTechList()
        {
            ...
            TechItemIlist<TechItem> techList = KCTGameStates.TechList;
        ...
        private static void DrawBLPlusWindow(int windowID)
        {
            ...
            if (KCTGameStates.ActiveKSC.Recon_Rollout.Find(rr => rr.RRType == Recon_Rollout.RolloutReconType.Rollout && rr.associatedID == b.Id.ToString()) != null && GUILayout.Button("Rollback"))
            {
                KCTGameStates.ActiveKSC.Recon_Rollout.Find(rr => rr.RRType == Recon_Rollout.RolloutReconType.Rollout && rr.associatedID == b.Id.ToString()).SwapRolloutType();
                GUIStates.ShowBLPlus = false;
            }
