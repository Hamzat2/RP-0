...
namespace KerbalConstructionTime
{
    public class KerbalConstructionTime : MonoBehaviour
    {
        ...
        internal void FacilityContextMenuSpawn(KSCFacilityContextMenu menu)
        {
            ...
            KSCContextMenuOverrider overrider = new KSCContextMenuOverrider(menu);
        ...
        public void Start()
        {
            ...
            switch (HighLogic.LoadedScene)
            {
                ...
                case GameScenes.FLIGHT:
                    ...
                    KCT_GUI.HideAll();
                    if (KCTGameStates.LaunchedVessel != null && FlightGlobals.ActiveVessel != null && FlightGlobals.ActiveVessel.situation == Vessel.Situations.PRELAUNCH)
                    {
                        ...
                        Recon_Rollout rollout = KCTGameStates.ActiveKSC.Recon_Rollout.FirstOrDefault(r => r.associatedID == KCTGameStates.LaunchedVessel.Id.ToString());
                        if (rollout != null)
        ...
        IEnumerator HandleEditorButton_Coroutine()
        {
            while (true)
            {
                ...
                yield return _wfsHalf; // new WaitForSeconds(0.5f);
            }
        }
        ...
        public void FixedUpdate()
        {
            ...
            try
            {
                ...
                if (!KCT_GUI.IsPrimarilyDisabled && (HighLogic.LoadedScene == GameScenes.FLIGHT || HighLogic.LoadedScene == GameScenes.SPACECENTER || HighLogic.LoadedScene == GameScenes.TRACKSTATION))
                {
                    ProcessWarp(lastUT);
                }
        ...
        {
            IKCTBuildItem ikctItem = Utilities.NextThingToFinish();
            if (KCTGameStates.TargetedItem == null && ikctItem != null) KCTGameStates.TargetedItem = ikctItem;
            ...
            if (KCTGameStates.CanWarp && ikctItem != null && !ikctItem.IsComplete())
            {
                ...
                {
                    if (ikctItem == KCTGameStates.TargetedItem && warpRate > 0 && TimeWarp.fetch.warpRates[warpRate] * dT * nBuffers > Math.Max(remaining, 0))
                    {
            ...
            else if (ikctItem != null && ikctItem == KCTGameStates.TargetedItem && (KCTGameStates.WarpInitiated || KCTGameStates.Settings.ForceStopWarp) && TimeWarp.CurrentRateIndex > 0 && (remaining < 1) && (!ikctItem.IsComplete())) //Still warp down even if we don't control the clock
            {
            ...
            if (HighLogic.LoadedScene == GameScenes.SPACECENTER)
            {
                ...
                foreach (KSCItem ksc in KCTGameStates.KSCs)
                {
                    for (int i = 0; i < ksc.Recon_Rollout.Count; i++)
                    {
                        ...
                        if (rr.RRType != Recon_Rollout.RolloutReconType.Reconditioning && Utilities.FindBLVesselByID(new Guid(rr.associatedID)) == null)
                        {
