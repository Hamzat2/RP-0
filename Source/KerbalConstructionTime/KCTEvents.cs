    ...
    public class KCTEvents
    {
        ...
        public void TechUnlockEvent(GameEvents.HostTargetAction<RDTech, RDTech.OperationResult> ev)
        {
            ...
            if (ev.target == RDTech.OperationResult.Successful)
            {
                ...
                else
                {
                    ...
                    ScreenMessages.PostScreenMessage("[KCT] It will unlock in " + MagiCore.Utilities.GetFormattedTime((KCTGameStates.TechList.First(t => t.TechID == ev.host.techID)).TimeLeft), 4.0f, ScreenMessageStyle.UPPER_LEFT);
                }
            }
        }
        ...
        public void GameSceneEvent(GameScenes scene)
        {
            if (scene == GameScenes.MAINMENU)
            {
                ...
                KCTGameStates.ActiveKSCName = "Stock";
                KCTGameStates.ActiveKSC = new KSCItem("Stock");
            ...
            if (ev.from == Vessel.Situations.PRELAUNCH && ev.host == FlightGlobals.ActiveVessel)
            {
                    ...
                    PresetManager.Instance.ActivePreset.generalSettings.ReconditioningTimes)
                {
                    if (HighLogic.CurrentGame.editorFacility == EditorFacility.VAB)
                    {
                        ...
                        KCTGameStates.ActiveKSC.Recon_Rollout.Add(new Recon_Rollout(ev.host, Recon_Rollout.RolloutReconType.Reconditioning, ev.host.id.ToString(), launchSite));
                    }
                }
            }
        }
        ...
        public void VesselRecoverEvent(ProtoVessel v, bool unknownAsOfNow)
        {
            ...
            if (!v.vesselRef.isEVA)
            {
                if (KCTGameStates.RecoveredVessel != null && v.vesselName == KCTGameStates.RecoveredVessel.ShipName)
                {
                    ...
                    KCTGameStates.ActiveKSC.Recon_Rollout.Add(new Recon_Rollout(KCTGameStates.RecoveredVessel, Recon_Rollout.RolloutReconType.Recovery, KCTGameStates.RecoveredVessel.Id.ToString()));
