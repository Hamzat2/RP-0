...
namespace KerbalConstructionTime
{
    public class BuildListVessel : IKCTBuildItem
    {
        ...
        {
            if (_ship != null && _ship.Parts != null && _ship.Parts.Count > 0) //If the parts are there, then the ship is loaded
            {
                return _ship;
            }
        ...
        public List<string> MeetsFacilityRequirements(bool highestFacility = true)
        {
            ...
            if (Type == ListType.VAB)
            {
                ...
                float launchpadNormalizedLevel = 1.0f * selectedPad.level / KCTGameStates.BuildingMaxLevelCache["LaunchPad"];
