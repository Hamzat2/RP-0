...
namespace KerbalConstructionTime
{
    public static class Utilities
    {
        ...
        public static void ProgressBuildTime()
        {
            ...
            if (UTDiff > 0 && (HighLogic.LoadedSceneIsEditor || UTDiff < (TimeWarp.fetch.warpRates[TimeWarp.fetch.warpRates.Length - 1] * 2)))
            {
                foreach (KSCItem ksc in KCTGameStates.KSCs)
                {
                    ...
                    ksc.Recon_Rollout.ForEach(delegate(Recon_Rollout rr)
                    {
                        if (rr.RRType == Recon_Rollout.RolloutReconType.Rollback && rr.IsComplete())
                        {
                    ...
                    ksc.Recon_Rollout.RemoveAll(rr => !PresetManager.Instance.ActivePreset.generalSettings.ReconditioningTimes || (rr.RRType != Recon_Rollout.RolloutReconType.Rollout && rr.IsComplete()));
        ...
        {
            IKCTBuildItem ship = NextThingToFinish();
            RampUpWarp(ship);
        }
        ...
        public static void RampUpWarp(IKCTBuildItem item)
        {
            ...
            if (double.IsPositiveInfinity(timeLeft))
                timeLeft = NextThingToFinish().GetTimeLeft();
            while ((newRate + 1 < TimeWarp.fetch.warpRates.Length) && (timeLeft > TimeWarp.fetch.warpRates[newRate + 1]*Planetarium.fetch.fixedDeltaTime) && (newRate < KCTGameStates.Settings.MaxTimeWarp))
            {
                newRate++;
            }
        ...
        public static bool ReconditioningActive(KSCItem KSC, string launchSite = "LaunchPad")
        {
            ...
            Recon_Rollout recon = KSC.GetReconditioning(launchSite);
            return (recon != null);
        }
        ...
        public static int TotalUpgradePoints()
        {
            ...
            total += (int)MathParser.GetStandardFormulaValue("UpgradesForScience", new Dictionary<string, string>() { { "N", KCTGameStates.SciPointsTotal.ToString() } });
            //Purchased funds
